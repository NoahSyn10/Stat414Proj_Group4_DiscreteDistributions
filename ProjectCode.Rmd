...
The built-in function dpois(x,λ) evaluates the probability density function (PDF) for a poisson distribution. For example, below the dpois() is used to determine the probability that an event will occur zero times in a specific time frame given that the rate of occurrence is λ = 5/2.
...
```
The built-in function ppois(x,λ) evaluates the cumulative density function (CDF) for a poisson distribution. For example, below the ppois() is used to determine the cumulative probability that an event will occur one time in a specific time frame given that the rate of occurrence is λ = 5/2.
